...
- `major` — `scripts/validate.mjs:363`: 재정의 금지 목록이 canonical 9종뿐이다.
  ...
  **[통합 라운드 — 해소]** `d62cc69`: colorTokens 하드코딩을 제거하고 system.css `:root`에서 토큰 73종을 자동 도출, 검사 대상을 "system.css 밖 모든 커스텀 프로퍼티 정의"로 확대했다. 재현 케이스 `.sidebar { --text-3: red }`가 실제로 잡히는 것을 음성 테스트로 확인. 검사 총량은 5260→5256건으로 줄었으나 커버리지는 색 토큰 9종 → 전 토큰·전 셀렉터·전 파일 + HTML 인라인 style로 넓어졌다 (D2 비약화).
- `nit` — `scripts/validate.mjs:364` `stripPrint`: `\n}`(열 0의 닫는 중괄호)를 블록 종료로 가정한다.
  ...
  **[통합 라운드 — 해소]** `d62cc69`: `stripPrint` 정규식을 중괄호 깊이 스캐너(`collectCustomProperties`)로 대체. 중첩 `@media print`·다중 print 블록·들여쓴 닫는 중괄호 음성 테스트 3종 모두 잡힌다.
