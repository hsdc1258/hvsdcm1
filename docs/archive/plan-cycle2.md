...
- **D7 — 철회됨 (2026-08-26, 사용자 지시로 게이팅으로 전환).** 사용자가 "로그인하지 않으면 학습과
  관련된 모든 내용을 전부 제거하고 그냥 개인 웹사이트처럼 보이게 하라"고 명시 지시하여 D7의 판단
  ("기존 동작 보존이므로 상시 노출 유지")을 뒤집었다. 미로그인 랜딩의 학습 콘텐츠 전부(상단바·푸터
  학습 링크, 히어로 학습 카피, 쇼케이스·동기화·통계 섹션, 드로어 학습 그룹, 학습 관련 메타/OG/타이틀
  문구)를 `<template data-study>`로 옮겨 미로그인 문서에는 렌더되지 않게 했고, 로그인 판정 후에만
  `home.js`가 주입한다. 이 계약은 `scripts/validate.mjs`의 `validateLandingGating()`이 정적으로 잠근다.
  기록 원문(위 D7)은 삭제하지 않고 남긴다.
