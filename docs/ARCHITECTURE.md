...
Competition submission routes use the distinct `COMPETITION_SUBMISSION_TOKEN`; ingest, usage, harness and moderator credentials cannot claim or update these jobs. Migration `0017` makes job identity and approved action immutable, permits only forward state transitions, and stores no generic result prose. The local `scripts/competition-submit.mjs` client reads an outside-Git config, checks the claimed action hash, request id and official destination against that config, and has no permissive generic form adapter. Unsupported or mismatched organizer flows terminate as a visible bounded `blocked` code. Only a deliberately added organizer-specific adapter can perform the single external attempt.
...
Competition submission coverage additionally runs migration `0017` against in-memory SQLite, races approval and claim operations, proves lease ownership and terminal replay behavior, and renders every submission state in the owner UI without a submit form. The local client tests cover no-job, running-before-action, one adapter call, bounded receipts, fail-closed unsupported flows, timeout, invalid responses and token redaction. The fast-lane selector test fixes the deterministic free asynchronous → remote-live → paid → on-site → unknown burden ordering while keeping total WIP at three.
