...
6. The local Codex control plane sends authenticated harness reports. The Worker returns retained tasks to the owner-only usage screen, which separates `active`, derived `stale`, and `complete` sessions. A selected session shows the original request, current/done/next facts, module progress, and verification artifacts without an organization chart or pipeline tree. A desktop `aside` shows Codex and Claude account-limit snapshots and reflows below the session list on narrower screens.
