...
9. The Behavior Lab static shell is noindexed and covered until `GET /api/behavior-lab/paper` authenticates the bearer session as the exact, separately configured `BEHAVIOR_OWNER_USERNAME`. The test-account list in `OWNER_USERNAME` is deliberately irrelevant: a missing or comma-separated Behavior owner fails closed, unauthenticated reads return 401, and non-owner sessions receive 404. The owner-only market tab calls `GET /api/behavior-lab/dashboard?symbol=...&period=...`; the Worker then constructs the exact eight accepted Bitget public-market GETs and returns only a fully validated live snapshot. The paper report POST routes three strict contracts to three independent reserved rows: the fixed single-session report, immutable three-arm `abc-paper-experiment-v1`, and exact six-arm `multi-paper-experiment-v2`. The six-arm row binds its exact experiment id, strategy-set hash, per-arm definition/policy/risk facts, six unique monotonic arm chains, shared public-feed chain, bounded histories, and 282,000-byte body cap. Owner GET prefers an active valid six-arm report and falls back to the three-arm row when it is absent. Neither row can downgrade or overwrite the other, and no path exposes a secret value, private exchange route, balance, or submission action.
10. The owner-only competition view places immutable approval requests before scan and candidate details. Each request binds a redacted review summary and exact approval wording to an action SHA-256. `POST /api/competitions/approvals/:requestId/decision` accepts one owner decision for only the latest report request, treats exact repeats idempotently, and rejects changed, expired or conflicting actions. Preparation approval is explicitly narrower than organizer consent or submission; sensitive approval kinds must expire within fifteen minutes and still require the local competition guard before transmission.
11. An approved latest `final_submission` decision atomically creates exactly one `competition_submission_jobs` row; every other approval kind and every hold creates none. A separately authenticated local executor claims one queued job with a five-minute lease and one-attempt ceiling, records `running` before an organizer adapter acts, then advances once to `succeeded`, `blocked`, or `submission_unknown`. An expired claim becomes `submission_unknown` rather than returning to the queue, so an ambiguous organizer response is never retried blindly. The owner view receives only the official URL, action/job identifiers, bounded result code, timestamps and an ASCII receipt reference; leases, raw receipts, PII, answers, consent text and submission bodies are never serialized there.
...
- `behavior-lab/`: owner-gated classic-script dashboard. `assets/js/app.js` reads the existing account bearer, keeps the shell covered until exact-owner verification, and renders separate market and active paper-experiment tabs. The paper tab refreshes every five seconds, hides completed experiments and the legacy single-session surface, prefers the six-arm v2 shape while retaining strict v1 fallback rendering, and shows each arm's accessible time-scaled bounded equity curve plus immutable policy/risk facts, position, trades, gate decisions, logs, metrics, and chain reference. `assets/js/core.js` owns the deterministic walk-forward and inert manual draft calculations.
...
Competition submission routes use the distinct `COMPETITION_SUBMISSION_TOKEN`; ingest, usage, harness and moderator credentials cannot claim or update these jobs. Migration `0017` adds the final request's public submission URL and canonical redacted action manifest to the deployed `0016` request table, then makes the queued job identity, manifest, rules URL, submission URL, and approval expiry immutable. The Worker recomputes the manifest SHA-256 before ingest, queues only a latest unexpired exact final approval, blocks expired queued/claimed work, rechecks expiry on `running`, and turns an abandoned claimed/running lease into terminal `submission_unknown` rather than requeueing it. The owner UI distinguishes rules from submission destination and displays organizer, contest/category, destination host, fee, rights class, consent/artifact hashes, and payload hash before the approval action. The local `scripts/competition-submit.mjs` client reads an outside-Git config, checks the claimed action hash, request id, rules URL and submission URL against that config, rechecks approval expiry immediately before an injected action, and has no permissive generic form adapter. Unsupported or mismatched organizer flows terminate as a visible bounded `blocked` code. Its exported claim/state helpers support a separately reviewed Browser executor in a five-minute operational heartbeat; the CLI remains fail closed, and only that deliberately added organizer-specific adapter can perform the single external attempt.
...
| `POST /api/competitions/submissions/claim` | competition submission token | Atomically lease one queued final-submission job after expiring ambiguous old leases |
| `POST /api/competitions/submissions/:jobId/state` | competition submission token | Advance one live lease through running to a bounded terminal result |
...
Competition submission coverage additionally runs migration `0017` against in-memory SQLite, races approval and claim operations, changes every manifest field while retaining the old hash, distinguishes rules and submit URLs, proves expiry at claim/running/action boundaries, and forces both claimed and running leases to expire into terminal non-requeueable unknown results. UI tests require every manifest fact/hash before approval and render every submission state without a submit form. The local client tests cover exported claim/state helpers, no-job, running-before-action, immediate pre-adapter expiry, one adapter call, bounded receipts, public-URL defenses, fail-closed unsupported flows, timeout, invalid responses and token redaction. The fast-lane selector test fixes the deterministic free asynchronous → remote-live → paid → on-site → unknown burden ordering while keeping total WIP at three.
