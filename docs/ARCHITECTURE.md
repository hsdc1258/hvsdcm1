...
- `behavior-lab/`: owner-gated classic-script dashboard. `assets/js/app.js` reads the existing account bearer, keeps the shell covered until exact-owner verification, and renders separate market and paper-session tabs. The paper tab refreshes its bounded summary every five seconds and, when v2 is present, shows engine cadence, stream freshness, champion/challenger metrics, promotion evidence and recent chained-audit references. `assets/js/core.js` owns the deterministic walk-forward and inert manual draft calculations.
