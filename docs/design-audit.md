...
- **시각 확인 (§6) — 부분 수행, 스크린샷 대조는 미완료**: 로컬 정적 서버(포트 4173)에
  ...
  **보완(리뷰 M-6, 6단계).** 이미지 캡처가 불가능한 조건은 그대로여서, 대신 **렌더된 DOM을
  독립 실행 HTML로 고정**했다. `docs/snapshots/diagrams.html`(13단원 21개 다이어그램 전부,
  각각 `단원ID — title (kind)` 소제목)과 `docs/snapshots/concept-sample.html`(III-01 개념 화면
  `#app` 전체)이며, CSS 두 개를 인라인해 파일 단독으로 열린다. 사람이 눈으로 조판을 볼 수 있는
  산출물이므로 스크린샷의 완전한 대체는 아니어도 계측보다는 §6에 가깝다.
  같은 라운드에서 계약 상한 데이터(label 8 / items 16 / center 8, kind별 items 개수 상한)로
  7종을 전수 렌더해 `getBBox()`로 재측정했다: 텍스트 130개, viewBox 이탈 0, 겹침 0.
  실데이터 21개(텍스트 288개)도 이탈 0 / 겹침 0으로 회귀가 없다.
