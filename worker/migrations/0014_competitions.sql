-- Competition reports are immutable daily snapshots. Discovery references stay separate from
-- organizer-controlled official URLs, and the database stores no application prose or PII.
...
CREATE TABLE competition_sources (
  ...
  FOREIGN KEY(idempotency_key) REFERENCES competition_reports(idempotency_key) ON DELETE RESTRICT,
...
CREATE TABLE competition_candidates (
  ...
  FOREIGN KEY(idempotency_key, source_id)
    REFERENCES competition_sources(idempotency_key, source_id) ON DELETE RESTRICT,
  CHECK(
