...
export function buildLearningPayloads({ root = ROOT, outputDirectory = DEFAULT_OUTPUT } = {}) {
  ...
  if (data?.UNITS?.length !== 4 || subunits.length !== 13 || data?.QUESTIONS?.length !== 78) {
    throw new Error('사회·문화 데이터 불변식(4단원/13중단원/78문항)이 깨졌습니다.');
  }
  if (!notebook?.NOTEBOOKS || Object.keys(explanations?.GUIDES || {}).length !== 13) {
  ...
  const referenced = new Set(data.QUESTIONS.map((question) => path.basename(question.image)));
  ...
  const manifest = {
    ...
    counts: { words: words.length, subunits: subunits.length, questions: data.QUESTIONS.length, images: imageNames.length },
    objects,
  };
