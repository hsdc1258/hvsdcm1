...
function browserHarness(responses) {
  ...
  window.fetch = async (url, options = {}) => {
    ...
    const response = () => new Response(JSON.stringify(entry.status === 200 ? { report: entry.report } : { error: entry.error || 'scripted error' }), {
...
try {
  {
    const responses = [
      { status: 200, report: paperReport(9, 101) },
      { status: 200, report: paperReport(10, 102, null) },
      { status: 200, report: paperReport(11, 103) },
    ];
