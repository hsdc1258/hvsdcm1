...
try {
  {
    ...
    await page.waitForSelector('#liveTradingEmpty:not([hidden])');
    assert.match(await page.locator('#liveTradingEmpty').textContent(), /BINANCE LIVE NOT CONNECTED/u);
    assert.equal(await page.locator('#liveTradingReport').getAttribute('hidden'), '');
    assert.doesNotMatch(await page.locator('#liveTabPanel').textContent(), /Bitget request failed/u);
