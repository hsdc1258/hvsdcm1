...
The paper read API remains rollout-compatible with the fixed single-session row, but the owner UI renders only a `starting` or `active` strict `abc-paper-experiment-v1` snapshot; completed experiments and the completed legacy card stay hidden. The A/B/C snapshot is stored under a separate D1 `usage_snapshots` source, requires the same dedicated ingest bearer, enforces exactly three independent 100 USDT arm chains plus one shared public-feed chain, rejects unknown/private/credential-shaped fields, and renders only after the human-owner gate. The UI shows the shared fingerprint, leaderboard, a bounded time-scaled equity/PnL curve (up to 64 immutable-chain-derived points), and per-arm position, trades, decisions, logs, return, drawdown, cost, win/loss, and chain details without adding any exchange submission surface.
