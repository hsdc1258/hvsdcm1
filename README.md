...
The paper tab remains backward compatible with the fixed single-session report and can additionally return one strict `abc-paper-experiment-v1` snapshot. The A/B/C snapshot is stored under a separate D1 `usage_snapshots` source, requires the same dedicated ingest bearer, enforces exactly three independent 100 USDT arm chains plus one shared public-feed chain, rejects unknown/private/credential-shaped fields, and renders only after the human-owner gate. The UI shows the shared fingerprint, leaderboard, per-arm equity/PnL/return/drawdown/cost/trades/open position, and bounded decisions/logs without adding any exchange submission surface.
